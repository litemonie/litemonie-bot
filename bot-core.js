...
function setupExpressServer(bot) {
  ...
  app.get('/', (req, res) => {
    res.status(200).json({ message: '🚀 Litemonie Bot is running!' });
  });
...
async function launchBot(useWebhook = false) {
  try {
    ...
    // ================================================================
    // SECTION 10.1: TEXT HANDLER - FIXED VERSION
    // ================================================================
    botInstance.on('text', async (ctx) => {
      ...
      const userId = ctx.from.id.toString();
      console.log(`📝 Processing text: "${text}" for user ${userId}`);

      // Check for deposit session
      ...
      if (depositSession && (depositSession.action === 'collect_email' || depositSession.action === 'collect_phone')) {
        console.log(`📝 Handling deposit session`);
      ...
      // Check for SendMoney session
      const sendMoneySession = sendMoney.sessionManager.getSession(userId);
      if (sendMoneySession) {
        console.log(`📝 Handling sendMoney session`);
        await sendMoney.handleText(ctx, text, getUsers(), getTransactions());
        await saveAllData();
        return;
      }
      
      // Check for upgrade recovery session - FIXED: using getUpgradeSession
      const recoverySession = await getUpgradeSession(userId);
      if (recoverySession && recoverySession.action === 'upgrade_recovery') {
        console.log(`📝 Handling upgrade recovery input`);
        await processRecoveryInput(ctx, text);
        return;
      }
      
      // No active session - ignore (don't send error message)
      console.log(`📝 No active session for user ${userId}, ignoring text`);
    });
